...
#    spec = UnispecProcessing(os.path.join(path, "config.txt"))
    ...
    for run in range(0,run_count):
        ...
        WP_end_data = [[[None], [None]] for item in range(0,stop_count[run])]
        sat = [[None], [None]]
        ...
        WP_end_data = spec.readFiles(spec.WP_end[run], spec.HeaderLines)
        ...
        avg_WP_start = spec.avgWPs(intdata_WPs)
        ...
        spec.writeOutput(R, spec.OutputPath, spec.OutputPrefix + dt.strftime("%Y-%m-%d__%H:%M:%S") + ".csv", Stop_data)
