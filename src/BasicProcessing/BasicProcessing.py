...
class UnispecProcessing:
    ...
    def __init__(self, config_file):
        ...
        self.WP_end_identifier = InputParams['WP_end_Identifier']
        self.WP_avg_mode = InputParams['WP_avg_mode']
...
#            Edited by A McMahon on 11/9/15 - Added check for empty files
            ...
            if file.endswith(".spu") and os.path.getsize(fullfile) > 0:
                ...
                else if file.endswith(self.WP_end_identifier + ".spu")
                    self.WP_end[run].append(fullfile)
                else:
                    #WP_break = True                        
...
#            Using "-1" prevents problems with different list lengths but ensures the data is out of the range interpolated so it is ignored
    ...
    def getDateTime(self, file):
        ...
        """
        dt_str = file[consts.header][consts.datetime].replace('"Time:    ','').replace('"\n','').split('  ')
    ...
    def create_WP_array(self,WP_data,WP_end_data,Stop_data=None):
        newdata = np.empty_like(Stop_data)
        
        if self.WP_avg_mode=="interpolate":
            pass
        else if self.WP_avg_mode=="switch":
            switching_point = len(Stop_data) / 2
            for s_idx, WP_stop in newdata:
                if s_idx < switching_point:
                    WP_stop = WP_data
                else:
                    WP_stop = WP_end_data
            
        else: #WP_avg_mode=="mean"
            for s_idx, WP_stop in newdata:
                avgWPs([WP_data, WP_end_data])
    ...
    def writeOutput(self,data,path,filename,Stop_data):
        """
        ...
        :param Stop_data: Original data array to extract timestamps from
        :type Array
        :returns: 0
        :rtype: Integer
        ...
        #Write header
        fh.write("Stop,Timestamp," + ",".join(['%f' % num for num in data[0,0]])) #str(data[0,0])[2:-2].replace("  ", ",").replace(" ","").replace("\n",""))
       
        for s_idx, stop in enumerate(data):
            fh.write("\n" + str(s_idx+1) + "," + self.getDateTime(Stop_data[s_idx]).strftime("%Y-%m-%d %H:%M:%S") + ",".join(['%f' % num for num in stop[1]])) #str(stop[1])[2:-2].replace("  ", ",").replace(" ","").replace("\n",""))
